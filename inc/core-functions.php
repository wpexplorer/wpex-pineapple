...
function wpex_get_post_layout() {
	...
	elseif ( is_home() || is_front_page() ) {
		$layout = wpex_get_theme_mod( 'home_layout', 'full-width' );
	}

	// Search
	elseif ( is_search() ) {
		$layout = wpex_get_theme_mod( 'search_layout', 'full-width' );
	}

	// Archive
	elseif ( is_archive() ) {
		$layout = wpex_get_theme_mod( 'archives_layout', 'full-width' );
	}
...
function wpex_get_entry_excerpt_length() {
	return apply_filters( 'wpex_get_entry_excerpt_length', 80 );
}
